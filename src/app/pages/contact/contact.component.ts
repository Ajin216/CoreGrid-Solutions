import { Component } from '@angular/core';
import { FormsModule, NgForm } from '@angular/forms'; // MANDATORY FOR FORMS

@Component({
  selector: 'app-contact',
  standalone: true,
  imports: [FormsModule],
  templateUrl: './contact.component.html'
})
export class ContactComponent {

  // 1. Define the data object to hold user input
  formData = {
    companyName: '',
    firstName: '',
    lastName: '',
    email: '',
    phone: '',
    service: '',
    details: ''
  };

  // 2. Put your agency's actual WhatsApp number here (Include country code, no + or spaces)
  agencyWhatsAppNumber = '919074501397';

  // 3. The execution function
  submitToWhatsApp(form: NgForm) {
    if (form.invalid) {
      // Mark all controls as touched to trigger real-time error displays
      Object.keys(form.controls).forEach(key => {
        form.controls[key].markAsTouched();
      });
      return;
    }

    // Construct the message format ( %0A creates a line break in URLs )
    const message = `*New CoreGrid Lead!*%0A%0A` +
      `*Company:* ${this.formData.companyName || 'Not specified'}%0A` +
      `*Name:* ${this.formData.firstName} ${this.formData.lastName || ''}%0A` +
      `*Email:* ${this.formData.email || 'Not specified'}%0A` +
      `*Phone:* ${this.formData.phone}%0A` +
      `*Service Needed:* ${this.formData.service}%0A` +
      `*Project Details:* ${this.formData.details || 'Not specified'}`;

    // Open WhatsApp in a new tab
    const whatsappUrl = `https://wa.me/${this.agencyWhatsAppNumber}?text=${message}`;
    window.open(whatsappUrl, '_blank');
  }
}